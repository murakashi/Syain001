...
public class DBCls1 {
	...
	public String selectMax() {
		...
		try {
			...
			//クエリ取得
			String sql = "select max(社員ID) + 1 as 最大社員ID from 社員マスタ";
	...
	/**************そのユーザーの権限を返す*****************************************/
	public String getKengrn(String w) {
		...
		try {
			...
			//クエリ取得
			sql = "select * from 社員マスタ where 社員ID = "+ w;
			//問い合わせの実行
			ResultSet rs = stmt.executeQuery(sql);
	...
	/**************そのユーザーの削除フラグを返す*****************************************/
	public String getFlag(String w) {
